...
class NMEAParser:
    ...
    def is_valid_sentence(self, sentence):
        ...
        if len(sentence) > self.nmea_max_length:
            self._logwarn(
                "Received invalid NMEA sentence. Max length is {}, but sentence was {} bytes".format(
                    self.nmea_max_length, len(sentence)
                )
            )
        ...
        if len(sentence) < self.nmea_min_length:
            self._logwarn(
                "Received invalid NMEA sentence. We need at least {} bytes to parse but got {} bytes".format(
                    self.nmea_min_length, len(sentence)
                )
            )
            self._logwarn("Sentence: {}".format(sentence))
            return False
        if sentence[0] != "$" and sentence[0] != "!":
            self._logwarn(
                'Received invalid NMEA sentence. Sentence should begin with "$" or "!", but instead begins with {}'.format(
                    sentence[0]
                )
            )
            self._logwarn("Sentence: {}".format(sentence))
        ...
        if _NMEA_CHECKSUM_SEPERATOR not in sentence:
            self._logwarn(
                'Received invalid NMEA sentence. Sentence should have a "{}" character to seperate the checksum, but we could not find it.'.format(
                    _NMEA_CHECKSUM_SEPERATOR
                )
            )
            self._logwarn("Sentence: {}".format(sentence))
