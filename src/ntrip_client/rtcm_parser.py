        ...
        while index < len(combined_buffer):
            ...
            if combined_buffer[index] == _RTCM_3_2_PREAMBLE:
                ...
                if len(combined_buffer) <= index + 2:
                    self._logdebug(
                        "Found beginning of RTCM packet at {}, but there is not enough data in the buffer to find the message length".format(
                            index
                        )
                    )
                ...
                message_length = (
                    combined_buffer[index + 1] << 8 | combined_buffer[index + 2]
                ) & 0x03FF
                if index + message_length + 6 <= len(combined_buffer):
                    ...
                    self._logdebug(
                        "Found beginning of RTCM packet at {}, but there is not enough data in the buffer to extract it, caching".format(
                            index
                        )
                    )
                    self._caching_data = True
